...
embedded in large Design of Experiments (DOE) studies, where hundreds or
thousands of parameterized runs are generated programmatically to
explore timing, density, effort, library variants, and other design
variables.
...
-   **PFXStudy**\
    ...
    GUI) that manages DOE workflows, scheduling, licensing, and
    aggregation.
...
A historical run is replayed locally for debugging.

**Scenario: Library Comparison**\
Multiple Vt libraries are compared using harvested QoR metrics.
...
Interaction:
    ...
    User → PFXStudy → PFXCore → Tools → Summaries → PFXStudy
...
### 4.12 Representation Strategy

-   In-memory: Strongly typed C++ classes
-   On-disk: TOML, JSON, CSV
