...
- `run` will look for requird collatral files (`run.toml`, `pipeline.tom`, etc.) and proceed with execution
- `status` will interrogate the `status.json` files available and report the current stage
...
All times to be recorded in local time.
