...
- `status` will interrogate the `status.json` files available and report the last recorded stage and success/failure of that stage. If no `status.json` files exist at all, report "no status available". 
...
order = 99
depends_on = ["route"]
inputs = ["stages/40_route/outputs/design_placed.enc"]
...
```
<prefix>.<toml_key_path>  →  set pfx(<prefix>.<toml_key_path>) {value}
```
...
```
set pfx(pipeline.exec.a.b) {value}
set pfx(pipeline.exec.c.d) {anotherValue}
```
...
5. Record the presence or absence of all files listed in stage `outputs`.
...
A stage `status.json` that does not contain `end_time`, `exit_code` or `outputs` file information
shall be incomplete and no further pipeline execution can continue, except when the `--force` flag
is used to invoke PFXCore. 
