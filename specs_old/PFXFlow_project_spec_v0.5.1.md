...
-   2026-02-03 -- Updated pipeline.toml schema to support grouped stage execution (single tool invocation with multiple checkpoints) via exec_group
    subsection
...
# Wrappers are invoked by the stage runner.
# - Ungrouped stage: <wrapper> <run_dir> <stage_name>
# - Grouped stages (exec_group): <wrapper> <run_dir> --exec-group <group> --target <stage_name> [--from <stage_name>]
[wrappers]
genus = "pfx_genus_run"
...
**Grouped execution (exec_group):**

-   When the user targets a stage `T`, the stage runner computes the prerequisite DAG closure as usual.
-   If some required stages share the same `exec_group`, the stage runner MAY execute them in a single wrapper invocation.
-   The wrapper is invoked once with a `--target` corresponding to the highest-order stage required within that group.
-   The wrapper MUST write each stage’s declared `outputs` into that stage’s own directory (`stages/<NN_name>/outputs/`), even though execution happened in one tool process.
-   After the wrapper returns successfully, the stage runner creates/updates `current/` exports **in dependency order** for each stage completed by the grouped run.
-   If the wrapper supports partial stop points, the stage runner MAY request `--target place` (for example) to run `init` and `place` but not `cts/route`.
-   A grouped stage is considered complete under the same definition as ungrouped stages: `status.json` indicates success and all declared `outputs` exist.
...
-   accepting invocation from the stage runner in one of these forms:
    -   **Ungrouped stage:** `<wrapper> <run_dir> <stage_name>`
    -   **Grouped stages:** `<wrapper> <run_dir> --exec-group <group> --target <stage_name> [--from <stage_name>]`
        -   `--target` specifies the highest stage to execute within the group (inclusive).
        -   `--from` (optional) requests a partial restart point within the group (if supported by the tool driver).
...
### 4.16 Implementation Mapping
  ...
  ---------------------------------------------------------------------
-   in grouped mode, writing logs/reports/outputs into each logical stage directory (`stages/<NN_name>/...`) that the group is responsible for
