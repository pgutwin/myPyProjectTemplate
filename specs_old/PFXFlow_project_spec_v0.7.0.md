...
-   2026-02-04 -- Tier-2 policies: tech catalog governance, core metrics
    harvesting, per-study append-only index w/ CLI filters,
    single-parent templates (no pipeline override), per-study resource
    limits, schema migration; formalized `<semantic path>`{=html}
...
-   Templates SHALL NOT redefine the pipeline in v1 (pipeline selection
    ...
    runs/ `<semantic DOE hierarchy>`{=html}/
...
**Technology bundle governance (v1):**

-   The technology bundle catalog is stored in a Git-managed repository
    (exact hosting and ownership is site-specific and TBD).
-   PFXFlow does not enforce catalog versioning; standard Git workflows
    (branching, tagging, reviews) are the source of truth.
-   Users MAY create and use private bundles and MAY mix-and-match
    private and shared bundles in a single study.
-   Changes to bundles do not mutate historical studies by default.
    Rebuilding a study after a bundle change is permitted and expected
    when desired.
...
**Library set policy (v1):**

-   Each run uses exactly **one active timing library set** (e.g., one
    selected standard-cell library variant) as defined by the selected
    technology bundle.
-   The active set is defined in the technology bundle section (not in
    the design section and not in the pipeline) to keep the design
    contract stable across technologies.
-   Multi-library mixing (e.g., simultaneous SVT/LVT/HVT selection per
    run) is out of scope for v1 and may be added later as an extension
    to the technology schema.
...
#### `<semantic path>`{=html} definition (v1)
...
A `<semantic path>` is a relative path rooted at the study directory
that encodes the DOE variable assignments for a run.

**Grammar (conceptual):**

    <semantic path> := <study_name> "/" <axis_dir>+ "/" <run_leaf>
...
For a study named `den_sweep` with axis `density ∈ {0.50, 0.55, 0.60}`:
    ...
    den_sweep/density=0.50/r0001/
    den_sweep/density=0.55/r0002/

For a 2D sweep with `clock_ps` and `density`:

    clk_den/clock_ps=320/density=0.50/r0042/
