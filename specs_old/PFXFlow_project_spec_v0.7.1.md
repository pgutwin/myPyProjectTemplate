...
**Study-root pipeline.toml (v1):**

-   `pipeline.toml` SHALL exist in the **study root directory** and
    defines the default pipeline for the entire study.
-   Per-run overrides are permitted by placing a `pipeline.toml` (or
    `pipeline_override.toml`) inside an individual run directory.
    -   When present, the per-run pipeline definition overrides the
        study pipeline for that run only.
