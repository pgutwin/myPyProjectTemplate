...
-   2026-02-05 -- Clarified run-level scheduling vs stage-level
    execution responsibilities between PFXStudy and PFXCore; defined
    run-as-job model (v0.8.4)
...
variables from `run.toml` are propagated into Tcl execution environments
via `pfx_vars.tcl`.

This mechanism is owned by **PFXCore** and is mandatory for v1.

Wrappers SHALL NOT parse `run.toml` or perform variable translation.

### 3.12.2 Responsibility Assignment

#### PFXStudy

PFXStudy SHALL:

-   Generate `run.toml` in each leaf run directory as the fully
    instantiated configuration for that DOE point.
-   Populate all DOE axes, design identifiers, technology identifiers,
    and user-defined control variables.
-   Treat `run.toml` as the authoritative description of run intent and
    inputs.
...
-   Consume `run.toml` as an immutable input.
-   Generate `pfx_vars.tcl` prior to first stage execution.
-   Place `pfx_vars.tcl` in each stage working directory.
-   Source `pfx_vars.tcl` before invoking any tool Tcl.
-   Regenerate `pfx_vars.tcl` only when `--force` is specified.

#### Wrappers

Wrappers SHALL:

-   Receive explicit stage context from the stage runner.
-   Change directory to the provided stage directory.
-   Execute the tool using generated Tcl drivers.
-   NOT parse `run.toml`.
-   NOT generate or modify `pfx_vars.tcl`.

### 3.12.3 Stage Invocation Context

Wrappers SHALL be invoked as:

    <wrapper> <run_dir> <stage_name> <stage_dir>

Where:

-   `run_dir` is the absolute path to the run directory.
-   `stage_name` is the logical pipeline stage name.
-   `stage_dir` is the absolute path to `stages/<NN>_<stage>/`.

Wrappers MUST use `stage_dir` as the working directory and SHALL NOT
infer directory structure.

### 3.12.4 run.toml Variable Contract (v1)

All top-level sections in `run.toml` SHALL be translated to Tcl
variables.
...
### 3.12.5 Tcl Namespace

All variables SHALL be placed in a single associative array:

    pfx(...)

### 3.12.6 Variable Naming

Variables SHALL be named:

    pfx(<section>.<key>)

Nested tables use dot concatenation.

### 3.12.7 Type Mapping Rules
  ...
  Float          float literal
  Boolean        0 / 1
  String         escaped Tcl string
  Array          Tcl list
  Inline table   JSON-encoded string
  Table          Flattened via dot-notation

### 3.12.8 String Escaping

PFXCore SHALL:

-   Prefer brace-quoted strings `{...}` when safe.
-   Fall back to double-quoted strings with escaping when required.
-   Escape backslash, quote, dollar, and newline.

### 3.12.9 Reserved Variables
...
-   Generated once during initial preparation.
-   Not regenerated on normal restart.
-   Regenerated only with `--force`.
-   Reused across stage reruns.

### 3.12.12 Compliance

All v1-compliant implementations MUST implement this model.

------------------------------------------------------------------------

## 3.13 Run Scheduling and Execution Responsibility Model (v1)

### 3.13.1 Overview

Runs are independent units of work and may execute in any order without
affecting DOE fidelity. Stages within a run form a strict dependency
chain and SHALL be executed sequentially.

This section defines the separation of responsibility between PFXStudy
and PFXCore.

### 3.13.2 PFXStudy Responsibilities (Run-Level Scheduling)

PFXStudy SHALL be responsible for:

-   Selecting which runs to execute.
-   Enforcing license and concurrency limits.
-   Launching runs locally or via a scheduler.
-   Ensuring required resources are available before launch.

Scheduling is performed at run granularity in v1.

### 3.13.3 PFXCore Responsibilities (Stage-Level Execution)

PFXCore SHALL be responsible for:

-   Enforcing stage dependencies and ordering.
-   Preparing stage directories and collateral.
-   Generating `pfx_vars.tcl`.
-   Invoking wrappers.
-   Recording `status.json` and summaries.
-   Terminating on failure.

Once invoked, PFXCore assumes resources are available for the full run.

### 3.13.4 Execution Model (v1)

1.  PFXStudy launches a run.
2.  PFXCore executes all stages sequentially.
3.  PFXCore exits with success/failure.
4.  PFXStudy observes completion.

PFXCore SHALL NOT submit cluster jobs in v1.

### 3.13.5 Implications

-   Parallelism exists only at run level.
-   Intra-run parallelism is tool-managed.
-   Partial multi-job runs are not supported.

### 3.13.6 Future Extensions (Non-Normative)

Future versions may introduce per-stage scheduling while preserving this
separation.
