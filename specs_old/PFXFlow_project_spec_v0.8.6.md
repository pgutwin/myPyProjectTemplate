...
Run directories are allocated by PFXStudy and populated by PFXCore.
    ...
    <study_name>/runs/<semantic path>/
      ...
      run.toml
      env.sh
      config/                       # generated config and drivers
      scripts/                      # tool control Tcl (provided by PFXStudy)
        pipeline_driver.tcl
...
### 3.7.2.1 Stage Execution Model (v1)

In v1, stages are executed through a generated shell launcher script
`stage_launch.sh` located in each stage directory:

    stages/<NN>_<stage>/stage_launch.sh

This script is generated by PFXCore and is the sole execution boundary
for a stage.

Wrappers are not used in v1.

Each `stage_launch.sh` SHALL:

1.  Change directory to the stage directory.
2.  Source `../../env.sh`.
3.  Invoke the tool using the command defined in `pipeline.toml`.
4.  Redirect output to declared log files.
5.  Propagate the tool exit status.
6.  Exit non-zero on failure.

### 3.7.2.2 pipeline.toml Execution Fields (v1)

Each stage SHALL define explicit execution metadata.
...
-   `exec.argv` (required): Exact argument vector used to invoke the
    tool.
-   `exec.primary_log` (optional): Primary log file relative to stage
    dir.
-   `exec.env` (optional): Map of environment overrides.

The referenced Tcl control file SHALL source `pfx_vars.tcl`.

PFXCore SHALL use these fields to generate `stage_launch.sh`.
...
exports = ["current/design_placed.enc=stages/30_place/outputs/design_placed.enc"]
```
