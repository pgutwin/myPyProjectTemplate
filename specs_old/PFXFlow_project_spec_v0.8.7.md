...
Optional fields:
  ...
  default_target   string   Default terminal stage name
...
### 3.7.2.3 \[conventions\] Table (Optional)
  ...
  current_dir    string   "current"       Export directory
...
Optional fields:
  ...
  Field        Type           Default   Description
  ------------ -------------- --------- ---------------------------
  depends_on   array\[str\]   \[\]      Stage dependency list
  inputs       array\[str\]   \[\]      Input file patterns
  outputs      array\[str\]   \[\]      Declared output paths
  exports      array\[str\]   \[\]      Export mappings (dst=src)
...
-   All `outputs` and `exports` paths are relative to `run_dir`.
...
Each export SHALL be of the form:

    <dst>=<src>

Where:

-   `<src>` is a path under the stage directory.
-   `<dst>` is a path under `current/`.

On successful stage completion, PFXCore SHALL create or update exports.
