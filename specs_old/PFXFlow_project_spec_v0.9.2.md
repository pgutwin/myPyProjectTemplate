...
-   2026-02-05 -- Restored and formalized variable propagation; defined
    ...
    clarified logging and execution semantics (v0.9.2)
...
`<study_name>`/runs/`<semantic path>`/ request.toml run.toml env.sh
config/ \# generated config and drivers scripts/ \# tool control Tcl
(provided by PFXStudy) pipeline_driver.tcl tool\_`<stage>`.tcl \#
optional (stage-specific Tcl) stages/ 10_synth/ logs/ reports/ outputs/
