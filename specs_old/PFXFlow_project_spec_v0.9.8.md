...
-   2026-02-05 -- Restored and formalized variable propagation; defined
    ...
    clarified logging and execution semantics (v0.9.8)
...
`<study_name>`/ study.toml pipeline.toml \# required (study-wide
...
`<study_name>`/runs/`<semantic path>`/ request.toml run.toml env.sh is
generated by PFXStudy and MUST exist before PFXCore execution. config/
\# generated config and drivers scripts/ \# tool control Tcl (provided
by PFXStudy) pipeline_driver.tcl tool\_`<stage>`.tcl \# optional
(stage-specific Tcl) stages/ 10_synth/ logs/ reports/ outputs/
status.json 20_init/ 30_place/ 40_cts/ 50_route/ 90_harvest/ \# stable
handoff links results/ run_summary.json run_summary.csv meta/ run_id.txt
run_intent.json provenance.json inputs_manifest.json
...
| env.sh is generated by | dy and | exist before PFXCore execution. |
| PFXStu | MUST |  |
| config/pipeline_driver.tcl | PFXCore | Entry Tcl / driver script |
...
| --- | --- | --- | --- |
| stages_dir | string | "stages" | Stage directory root |
| status_file | string | "status.json" | Stage status filename |
| (stage-relative) |  |  |  |
...
| Field | Type | Description |
| --- | --- | --- |
| primary_log | string | Primary log path (stage-relative) |
| env | table\[string\] | Environment overrides |


### 3.7.2.6 Path Resolution Rules

-   All declared `outputs` paths are relative to the stage working directory `stages/<NN>_<stage>/`.
...
outputs = ["results/harvest.json"]

[stage.exec]
argv = ["python3", "../../scripts/harvest.py", "--in", "stages/30_place/outputs/design_placed.enc", "--out", "results/harvest.json"]
primary_log = "harvest.log"
```
...
- Keys may contain only letters, digits, dots (`.`), and dashes (`-`)
